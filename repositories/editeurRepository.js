...
export const editeurRepository = {
  async findAllEditeurs() {
    ...
    try{
      ...
      return rows.map(row => new Editeur(
        row.id,
        row.editeur       
      ));      
    }
    catch{
      return { success: false, data: [], message: "Error || editeurRepository || findAllEditeurs:" + error };
    };  
  },
